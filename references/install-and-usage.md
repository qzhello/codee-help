...
```bash
mkdir -p .claude/commands
cp /path/to/codee-help/examples/claude-command.codee.md .claude/commands/codee.md
```

## 3. Gemini CLI

Gemini CLI 推荐用自定义命令目录：

- 全局：`~/.gemini/commands/`
- 项目内：`<project-root>/.gemini/commands/`
...
```bash
mkdir -p .gemini/commands
cp /path/to/codee-help/examples/gemini-command.codee.toml .gemini/commands/codee.toml
```
